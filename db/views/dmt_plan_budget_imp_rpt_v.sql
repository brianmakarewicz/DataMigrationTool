-- DMT_PLAN_BUDGET_IMP_RPT_V
CREATE OR REPLACE FORCE EDITIONABLE VIEW "DMT_PLAN_BUDGET_IMP_RPT_V" ("INTEGRATION_ID", "ORCHESTRATION_CODE", "RUN_STATUS", "START_DATE", "END_DATE", "PREFIX", "STG_SEQUENCE_ID", "TFM_SEQUENCE_ID", "SCENARIO", "VERSION", "ENTITY", "ACCOUNT", "PERIOD", "AMOUNT", "CURRENCY", "STG_STATUS", "TFM_STATUS", "EFFECTIVE_STATUS", "RECONCILIATION_STATUS", "STG_ERRORS", "TFM_ERRORS", "ALL_ERRORS", "RESULTS_UPDATED_DATE", "STAGE_DATE", "SOURCE_RECORD_ID")  AS 
  SELECT
    t.INTEGRATION_ID,
...
FROM
    DMT_OWNER.DMT_PLAN_BUDGET_STG_TBL           s
    LEFT JOIN DMT_OWNER.DMT_PLAN_BUDGET_TFM_TBL t
