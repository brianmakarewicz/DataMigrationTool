...
CREATE OR REPLACE FORCE EDITIONABLE VIEW "DMT_PIPELINE_SUMMARY_V" ("RUN_ID", "INTEGRATION_ID", "SCENARIO_ID", "PIPELINE_CODES", "SCENARIO_NAME", "PREFIX", "SUBMITTED_DATE", "COMPLETED_DATE", "RUN_STATUS", "PIPELINE", "OBJECT_TYPE", "SORT_ORDER", "OBJECT_STATUS", "TOTAL_ROWS", "LOADED_ROWS", "FAILED_ROWS", "GENERATED_ROWS", "IN_PROGRESS_ROWS", "UNRECONCILED_ROWS", "CARD_SUMMARY", "CARD_CSS_CLASS", "LOAD_ESS_JOB_ID", "IMPORT_ESS_JOB_ID", "POSTRUN_ESS_JOB_ID", "ZIP_FILENAME")  AS 
...
pivoted AS (
    SELECT
        ...
        SUM(CASE WHEN cs.STATUS = 'LOADED'    THEN cs.ROW_COUNT ELSE 0 END) AS LOADED_ROWS,
        SUM(CASE WHEN cs.STATUS = 'FAILED'    THEN cs.ROW_COUNT ELSE 0 END) AS FAILED_ROWS,
        SUM(CASE WHEN cs.STATUS = 'GENERATED' THEN cs.ROW_COUNT ELSE 0 END) AS GENERATED_ROWS,
        SUM(CASE WHEN cs.STATUS NOT IN ('LOADED','FAILED','GENERATED')
