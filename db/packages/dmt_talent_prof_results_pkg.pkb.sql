...
-- ============================================================
    ...
    -- ProfileItem records keep their per-record HDL error path (RECONCILE_HDL
    -- above); their base-tier proof is a separate future registration.
    -- --------------------------------------------------------
            ...
            FOR i IN 1 .. l_rows.COUNT LOOP
                IF l_rows(i).SOURCE_TYPE = 'BASE'
                   ...
                   AND l_rows(i).FUSION_ID IS NOT NULL THEN
                    ...
                    -- id. The ONLY path to LOADED. Static UPDATE.
    ...
    BEGIN
        ...
        -- 1. TalentProfile — Contract v1 base-table proof (design section 5).
        ...
        -- Contract v1 parser below: a TalentProfile row reaches LOADED only when
        -- the profile is positively confirmed in the Fusion base table
        -- (HRT_PROFILES_B) with a real profile id, which the parser stamps into
        -- FUSION_PROFILE_ID.
        DMT_HDL_UTIL_PKG.RECONCILE_HDL(
            p_run_id => p_run_id,
            p_request_id       => p_request_id,
            p_tfm_table        => 'DMT_TALENT_PROF_TFM_TBL',
            p_stg_table        => 'DMT_TALENT_PROF_STG_TBL',
        ...
        -- separate future registration for the child object).
        DMT_HDL_UTIL_PKG.RECONCILE_HDL(
            p_run_id => p_run_id,
            p_request_id     => p_request_id,
            p_tfm_table      => 'DMT_TALENT_PROF_ITEM_TFM_TBL',
