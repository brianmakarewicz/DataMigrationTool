-- PACKAGE BODY DMT_POZ_SUP_CONT_FBDI_GEN_PKG

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DMT_POZ_SUP_CONT_FBDI_GEN_PKG" AS
-- ============================================================
-- DMT_POZ_SUP_CONT_FBDI_GEN_PKG Body
-- Generates PozSupContactsInt.csv zipped for Fusion FBDI import.
-- Interface table: POZ_SUP_CONTACTS_INT
-- ============================================================

    C_PKG      CONSTANT VARCHAR2(50) := 'DMT_POZ_SUP_CONT_FBDI_GEN_PKG';
    C_CSV_FILE CONSTANT VARCHAR2(50) := 'PozSupContactsInt.csv';
    ...
    BEGIN
        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'Supplier Contact FBDI generation start.',
            p_package        => C_PKG,
            p_procedure      => C_PROC);
        ...
        FOR r IN (
            ...
            DBMS_LOB.APPEND(l_csv,
                ...
                q(r.LAST_NAME_NEW)          || ',' ||
                q(r.TITLE)                  || ',' ||
                q(r.PRIMARY_ADMIN_CONTACT)  || ',' ||
                q(r.EMAIL_ADDRESS)          || ',' ||
                q(r.EMAIL_ADDRESS_NEW)      || ',' ||
                ...
                q(r.FAX_AREA_CODE)          || ',' ||
                q(r.FAX)                    || ',' ||
                q(r.MOBILE_COUNTRY_CODE)    || ',' ||
                q(r.MOBILE_AREA_CODE)       || ',' ||
                q(r.MOBILE)                 || ',' ||
                qd(r.INACTIVE_DATE)         || ',' ||
                ...
                q(r.USER_ACCOUNT_ACTION) || ',' ||
                q(r.ROLE1)  || ',' || q(r.ROLE2)  || ',' ||
                q(r.ROLE3)  || ',' || q(r.ROLE4)  || ',' ||
                q(r.ROLE5)  || ',' || q(r.ROLE6)  || ',' ||
                q(r.ROLE7)  || ',' || q(r.ROLE8)  || ',' ||
                q(r.ROLE9)  || ',' || q(r.ROLE10)
                || l_crlf
            );
        END LOOP;
        ...
        IF l_row_count = 0 THEN
            DMT_UTIL_PKG.LOG(
                p_run_id => p_run_id,
                p_message        => 'No VALIDATED supplier contact rows found. Skipping zip generation.',
        ...
        INSERT INTO DMT_OWNER.DMT_FBDI_CSV_TBL (
            ...
            'SupplierContacts', C_CSV_FILE, l_row_count, l_csv, SYSDATE
        );
        DBMS_LOB.FREETEMPORARY(l_csv);
        ...
        x_filename := 'SupplierContacts_' || p_run_id || '.zip';
        ...
        INSERT INTO DMT_OWNER.DMT_FBDI_ZIP_TBL (
            ...
            (SELECT MAX(FBDI_CSV_ID) FROM DMT_OWNER.DMT_FBDI_CSV_TBL
             WHERE RUN_ID = p_run_id AND OBJECT_TYPE = 'SupplierContacts'),
            p_run_id, 'SupplierContacts', x_filename,
        ...
        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'Supplier Contact FBDI generation complete. Rows: ' || l_row_count ||
                                ' | File: ' || x_filename ||
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DMT_UTIL_PKG.LOG_ERROR(
                p_run_id => p_run_id,
                p_message        => 'Supplier Contact FBDI generation failed.',
                p_sqlerrm        => SQLERRM,
                p_package        => C_PKG,
                p_procedure      => C_PROC);
            RAISE;
