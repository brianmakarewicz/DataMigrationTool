...
-- ============================================================
    ...
    BEGIN
        ...
        -- Echo the reconciled outcome back to the STG table. Per section 5 the
        -- STG row carries the forward-only status that DOWNSTREAM objects read
        -- (e.g. the PO pre-validator gates on the supplier STG row being LOADED).
        -- The per-object reconciler above updates only TFM; without this echo the
        -- supplier STG row stays TRANSFORMED and every dependent PO/child is
        -- wrongly skipped even though the supplier actually loaded. One STG/TFM
        -- pair per supplier CEMLI, linked 1:1 by STG_SEQUENCE_ID (matches the
        -- echo pattern the PurchaseOrders and other reconcilers already use).
        IF p_cemli_code = 'Suppliers' THEN
            UPDATE DMT_OWNER.DMT_POZ_SUPPLIERS_STG_TBL stg SET stg.STG_STATUS='LOADED', stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUPPLIERS_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='LOADED') AND stg.STG_STATUS<>'LOADED';
            UPDATE DMT_OWNER.DMT_POZ_SUPPLIERS_STG_TBL stg SET stg.STG_STATUS='FAILED',
                   stg.ERROR_TEXT=DMT_UTIL_PKG.APPEND_ERROR(stg.ERROR_TEXT,(SELECT t.ERROR_TEXT FROM DMT_OWNER.DMT_POZ_SUPPLIERS_TFM_TBL t WHERE t.STG_SEQUENCE_ID=stg.STG_SEQUENCE_ID AND t.RUN_ID=p_run_id)), stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUPPLIERS_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='FAILED') AND stg.STG_STATUS NOT IN ('LOADED','FAILED');
        ELSIF p_cemli_code = 'SupplierAddresses' THEN
            UPDATE DMT_OWNER.DMT_POZ_SUP_ADDR_STG_TBL stg SET stg.STG_STATUS='LOADED', stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUP_ADDR_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='LOADED') AND stg.STG_STATUS<>'LOADED';
            UPDATE DMT_OWNER.DMT_POZ_SUP_ADDR_STG_TBL stg SET stg.STG_STATUS='FAILED',
                   stg.ERROR_TEXT=DMT_UTIL_PKG.APPEND_ERROR(stg.ERROR_TEXT,(SELECT t.ERROR_TEXT FROM DMT_OWNER.DMT_POZ_SUP_ADDR_TFM_TBL t WHERE t.STG_SEQUENCE_ID=stg.STG_SEQUENCE_ID AND t.RUN_ID=p_run_id)), stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUP_ADDR_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='FAILED') AND stg.STG_STATUS NOT IN ('LOADED','FAILED');
        ELSIF p_cemli_code = 'SupplierSites' THEN
            UPDATE DMT_OWNER.DMT_POZ_SUP_SITE_STG_TBL stg SET stg.STG_STATUS='LOADED', stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUP_SITE_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='LOADED') AND stg.STG_STATUS<>'LOADED';
            UPDATE DMT_OWNER.DMT_POZ_SUP_SITE_STG_TBL stg SET stg.STG_STATUS='FAILED',
                   stg.ERROR_TEXT=DMT_UTIL_PKG.APPEND_ERROR(stg.ERROR_TEXT,(SELECT t.ERROR_TEXT FROM DMT_OWNER.DMT_POZ_SUP_SITE_TFM_TBL t WHERE t.STG_SEQUENCE_ID=stg.STG_SEQUENCE_ID AND t.RUN_ID=p_run_id)), stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUP_SITE_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='FAILED') AND stg.STG_STATUS NOT IN ('LOADED','FAILED');
        ELSIF p_cemli_code = 'SupplierSiteAssignments' THEN
            UPDATE DMT_OWNER.DMT_POZ_SUP_SITE_ASSN_STG_TBL stg SET stg.STG_STATUS='LOADED', stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUP_SITE_ASSN_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='LOADED') AND stg.STG_STATUS<>'LOADED';
            UPDATE DMT_OWNER.DMT_POZ_SUP_SITE_ASSN_STG_TBL stg SET stg.STG_STATUS='FAILED',
                   stg.ERROR_TEXT=DMT_UTIL_PKG.APPEND_ERROR(stg.ERROR_TEXT,(SELECT t.ERROR_TEXT FROM DMT_OWNER.DMT_POZ_SUP_SITE_ASSN_TFM_TBL t WHERE t.STG_SEQUENCE_ID=stg.STG_SEQUENCE_ID AND t.RUN_ID=p_run_id)), stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUP_SITE_ASSN_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='FAILED') AND stg.STG_STATUS NOT IN ('LOADED','FAILED');
        ELSIF p_cemli_code = 'SupplierContacts' THEN
            UPDATE DMT_OWNER.DMT_POZ_SUP_CONTACTS_STG_TBL stg SET stg.STG_STATUS='LOADED', stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUP_CONTACTS_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='LOADED') AND stg.STG_STATUS<>'LOADED';
            UPDATE DMT_OWNER.DMT_POZ_SUP_CONTACTS_STG_TBL stg SET stg.STG_STATUS='FAILED',
                   stg.ERROR_TEXT=DMT_UTIL_PKG.APPEND_ERROR(stg.ERROR_TEXT,(SELECT t.ERROR_TEXT FROM DMT_OWNER.DMT_POZ_SUP_CONTACTS_TFM_TBL t WHERE t.STG_SEQUENCE_ID=stg.STG_SEQUENCE_ID AND t.RUN_ID=p_run_id)), stg.LAST_UPDATED_DATE=SYSDATE
            WHERE stg.STG_SEQUENCE_ID IN (SELECT t.STG_SEQUENCE_ID FROM DMT_OWNER.DMT_POZ_SUP_CONTACTS_TFM_TBL t WHERE t.RUN_ID=p_run_id AND t.TFM_STATUS='FAILED') AND stg.STG_STATUS NOT IN ('LOADED','FAILED');
        END IF;
