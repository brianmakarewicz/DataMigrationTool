-- PACKAGE BODY DMT_AP_FBDI_GEN_PKG

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DMT_AP_FBDI_GEN_PKG" 
AS
    C_PKG CONSTANT VARCHAR2(50) := 'DMT_AP_FBDI_GEN_PKG';

-- ============================================================
-- DMT_AP_FBDI_GEN_PKG body
-- APInvoices FBDI zip generation.
...
-- ============================================================
    ...
    BEGIN
        ...
        FOR r IN (
            SELECT
                '"' || TO_CHAR(TFM_SEQUENCE_ID) || '"' || ','  -- INVOICE_ID = TFM_SEQUENCE_ID (globally unique per run)
    ...
    BEGIN
        ...
        FOR r IN (
            SELECT
                ...
                || '"' || NVL(TO_CHAR(l.LINE_NUMBER), '') || '"' || ','
                || '"' || REPLACE(NVL(l.LINE_TYPE_LOOKUP_CODE,''), '"', '""') || '"' || ','
                || '"' || NVL(TO_CHAR(AMOUNT), '') || '"' || ','
    ...
    BEGIN
        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'AP Invoice FBDI generation start.',
            p_package        => C_PKG,
            p_procedure      => C_PROC);

        -- Filename: AP_{GroupValue}_{IntegrationID}.zip
        ...
        IF (l_hdr_csv IS NULL OR DBMS_LOB.GETLENGTH(l_hdr_csv) = 0)
           AND (l_lines_csv IS NULL OR DBMS_LOB.GETLENGTH(l_lines_csv) = 0) THEN
            DMT_UTIL_PKG.LOG(
                p_run_id => p_run_id,
                p_message        => 'No STAGED AP Invoice rows found. Skipping FBDI generation.',
        ...
        IF l_hdr_csv IS NOT NULL AND DBMS_LOB.GETLENGTH(l_hdr_csv) > 0 THEN
            DMT_UTIL_PKG.REGISTER_CSV(p_run_id, l_zip_id, 1, 'APInvoices', 'ApInvoicesInterface.csv',     0, l_hdr_csv, l_fbdi_csv_id);
        END IF;
        IF l_lines_csv IS NOT NULL AND DBMS_LOB.GETLENGTH(l_lines_csv) > 0 THEN
            DMT_UTIL_PKG.REGISTER_CSV(p_run_id, l_zip_id, 2, 'APInvoices', 'ApInvoiceLinesInterface.csv', 0, l_lines_csv, l_lines_csv_id);
        END IF;
        DMT_UTIL_PKG.BUILD_ZIP_FROM_CSVS(p_run_id, l_zip_id, 'APInvoices', x_filename, l_zip, l_bytes);
        ...
        x_fbdi_zip    := l_zip;
        x_fbdi_csv_id := NVL(l_fbdi_csv_id, l_lines_csv_id);  -- first registered id, matches the zip bridge pointer

        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'AP Invoice FBDI generation complete.' ||
                                ' | File: ' || x_filename ||
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DMT_UTIL_PKG.LOG_ERROR(
                p_run_id => p_run_id,
                p_message        => 'AP Invoice FBDI generation failed.',
                p_sqlerrm        => SQLERRM,
                p_package        => C_PKG,
                p_procedure      => C_PROC);
            RAISE;
