    ...
    BEGIN
        ...
        FOR r IN (
            ...
            IF r.import_status IN ('Y','PROCESSED','SUCCESS','COMPLETED') THEN
                UPDATE DMT_OWNER.DMT_PLAN_BUDGET_TFM_TBL SET STATUS='LOADED', RESULTS_UPDATED_DATE=SYSDATE, LAST_UPDATED_DATE=SYSDATE
                WHERE RUN_ID=p_run_id AND SCENARIO=r.scenario AND STATUS!='LOADED';
                l_loaded := l_loaded + SQL%ROWCOUNT;
            ELSE
                UPDATE DMT_OWNER.DMT_PLAN_BUDGET_TFM_TBL SET STATUS='FAILED',
                    ERROR_TEXT=DMT_UTIL_PKG.APPEND_ERROR(ERROR_TEXT,'[FUSION_ERROR] '||r.error_msg),
                    RESULTS_UPDATED_DATE=SYSDATE, LAST_UPDATED_DATE=SYSDATE
                WHERE RUN_ID=p_run_id AND SCENARIO=r.scenario AND STATUS!='FAILED';
        ...
        UPDATE DMT_OWNER.DMT_PLAN_BUDGET_STG_TBL SET STATUS='LOADED', LAST_UPDATED_DATE=SYSDATE
        WHERE STG_SEQUENCE_ID IN (SELECT STG_SEQUENCE_ID FROM DMT_OWNER.DMT_PLAN_BUDGET_TFM_TBL WHERE RUN_ID=p_run_id AND STATUS='LOADED');
        UPDATE DMT_OWNER.DMT_PLAN_BUDGET_STG_TBL SET STATUS='FAILED', LAST_UPDATED_DATE=SYSDATE
        WHERE STG_SEQUENCE_ID IN (SELECT STG_SEQUENCE_ID FROM DMT_OWNER.DMT_PLAN_BUDGET_TFM_TBL WHERE RUN_ID=p_run_id AND STATUS='FAILED');
