-- PACKAGE BODY DMT_WORK_SCHED_TRANSFORM_PKG

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DMT_WORK_SCHED_TRANSFORM_PKG" 
AS
-- ============================================================
-- DMT_WORK_SCHED_TRANSFORM_PKG Body
-- ============================================================

    C_PKG CONSTANT VARCHAR2(50) := 'DMT_WORK_SCHED_TRANSFORM_PKG';
    ...
    BEGIN
        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'TRANSFORM_WORKSCHEDULES start.',
            p_package        => C_PKG,
            p_procedure      => 'TRANSFORM_WORKSCHEDULES');

        l_prefix := get_prefix(p_run_id);

        -- WorkSchedule
        INSERT INTO DMT_OWNER.DMT_WORK_SCHED_TFM_TBL (
            TFM_SEQUENCE_ID,
            STG_SEQUENCE_ID,
            RUN_ID,
            FBDI_CSV_ID,
        ...
        SELECT
            DMT_OWNER.DMT_WORK_SCHED_TFM_SEQ.NEXTVAL,
            s.STG_SEQUENCE_ID,
            p_run_id,
            NULL,
            ...
            'STAGED',
            SYSDATE
        FROM DMT_OWNER.DMT_WORK_SCHED_STG_TBL s
        WHERE (
        ...
        AND NOT EXISTS (
            ...
            AND    e.SUB_OBJECT = 'Work Schedules'
        ...
        UPDATE DMT_OWNER.DMT_WORK_SCHED_STG_TBL
        ...
        -- WorkScheduleShift
        INSERT INTO DMT_OWNER.DMT_WORK_SCHED_DTL_TFM_TBL (
            TFM_SEQUENCE_ID,
            STG_SEQUENCE_ID,
            RUN_ID,
            FBDI_CSV_ID,
        ...
        SELECT
            DMT_OWNER.DMT_WORK_SCHED_DTL_TFM_SEQ.NEXTVAL,
            s.STG_SEQUENCE_ID,
            p_run_id,
            NULL,
            s.WORK_SCHEDULE_NAME,
            ...
            'STAGED',
            SYSDATE
        FROM DMT_OWNER.DMT_WORK_SCHED_DTL_STG_TBL s
        WHERE (
            (p_run_mode = 'NEW' AND s.STG_STATUS IN ('NEW', 'RETRY'))
            OR (p_run_mode = 'FAILED' AND s.STG_STATUS = 'FAILED')
            OR (p_run_mode = 'ALL')
        ...
        AND NOT EXISTS (
            SELECT 1 FROM DMT_OWNER.DMT_STG_TFM_ERROR_TBL e
            WHERE  e.RUN_ID = p_run_id
            AND    e.SUB_OBJECT = 'Schedule Details'
            AND    e.STG_SEQUENCE_ID = s.STG_SEQUENCE_ID)
        AND NOT EXISTS (
            SELECT 1
        ...
        UPDATE DMT_OWNER.DMT_WORK_SCHED_DTL_STG_TBL
        SET    STG_STATUS = 'TRANSFORMED', LAST_UPDATED_DATE = SYSDATE
        WHERE  STG_SEQUENCE_ID IN (
            SELECT STG_SEQUENCE_ID
        ...
        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'TRANSFORM_WORKSCHEDULES complete. Rows transformed: ' || l_ok_count,
            p_package        => C_PKG,
            p_procedure      => 'TRANSFORM_WORKSCHEDULES');
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DMT_UTIL_PKG.LOG_ERROR(
                p_run_id => p_run_id,
                p_message        => 'TRANSFORM_WORKSCHEDULES failed.',
                p_sqlerrm        => SQLERRM,
                p_package        => C_PKG,
                p_procedure      => 'TRANSFORM_WORKSCHEDULES');
            RAISE;
    END TRANSFORM_WORKSCHEDULES;
