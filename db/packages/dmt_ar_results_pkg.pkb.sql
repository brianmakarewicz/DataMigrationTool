...
-- ============================================================
    ...
    BEGIN
        ...
        --   literal '[RECONCILE_ERROR] ' tag.>>
                   '[RECONCILE_ERROR] AR invoice line not confirmed in Fusion '
                   || '(not found in RA_CUSTOMER_TRX_ALL / RA_CUSTOMER_TRX_LINES_ALL for '
                   || 'this run) after reconciliation; its import outcome could not be verified.'
        ...
        -- (EDIT-SCOPE deleted — DMT_RA_LINES_TFM_TBL is not shared.)
        ;
        ...
        -- <<EDIT-MSG — CHANGE BELOW: the message text. It MUST begin with the
        --   literal '[RECONCILE_ERROR] ' tag.>>
                   '[RECONCILE_ERROR] AR invoice distribution not confirmed in Fusion '
                   || '(not found in RA_CUST_TRX_LINE_GL_DIST_ALL for this run) after '
        ...
        AND    TFM_STATUS NOT IN ('LOADED','FAILED')
        AND    (p_work_queue_id IS NULL OR WORK_QUEUE_ID = p_work_queue_id)  -- work-queue-ID core: sweep only this item's rows
        -- (EDIT-SCOPE deleted — DMT_RA_DISTS_TFM_TBL is not shared.)
        ;
