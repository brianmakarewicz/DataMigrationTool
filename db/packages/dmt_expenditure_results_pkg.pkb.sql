...
-- ============================================================
    ...
    -- The Contract v1 base-tier positive proof for Expenditures — the SINGLE-TIER
    -- FBDI template (design section 5, Option A shape). This is the copy-template
    -- for the other single-tier FBDI objects (Items, GLBudgets, ProjectBudgets,
    ...
    BEGIN
        ...
        IF l_err_code <> DMT_UTIL_PKG.C_SUCCESS THEN
            RAISE_APPLICATION_ERROR(-20095,
                'APPLY_CONTRACT_V1_EXPENDITURES: Contract v1 fetch failed for '
                || 'Expenditures (detail in DMT_LOG_TBL).');
        END IF;
        ...
        IF l_rows.COUNT = 0 THEN
            ...
            DMT_UTIL_PKG.LOG(
                p_run_id    => p_run_id,
                p_message   => C_PROC || ': Expenditures recon report returned zero rows; '
            ...
            FOR i IN 1 .. l_rows.COUNT LOOP
                IF l_rows(i).SOURCE_TYPE = 'BASE'
                   ...
                   AND l_rows(i).FUSION_ID IS NOT NULL THEN
                    -- Positive proof: item found in PJC_EXP_ITEMS_ALL with a real
                    -- id. The ONLY path to LOADED. Static UPDATE keyed on RECON_KEY.
                    UPDATE DMT_PJC_EXPENDITURES_TFM_TBL
                    SET    TFM_STATUS                 = 'LOADED',
                           FUSION_EXPENDITURE_ITEM_ID = l_rows(i).FUSION_ID,
                           RESULTS_UPDATED_DATE       = SYSDATE,
                           LAST_UPDATED_DATE          = SYSDATE
                    WHERE  RUN_ID    = p_run_id
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DMT_UTIL_PKG.LOG_ERROR(
                ...
                p_procedure => C_PROC);
            RAISE;
    END APPLY_CONTRACT_V1_EXPENDITURES;
