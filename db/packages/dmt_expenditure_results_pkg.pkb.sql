            ...
            IF p_import_ess_id IS NOT NULL THEN
                ...
                BEGIN
                    ...
                    IF l_ir_xml IS NOT NULL AND DBMS_LOB.GETLENGTH(l_ir_xml) > 0 THEN
                        ...
                        -- Shared import-report match writer (backlog item 28):
                        -- single-key match on ORIG_TRANSACTION_REFERENCE, byte-
                        -- identical to the loop this replaces.
                        l_ir_matched := l_ir_matched + DMT_IMPORT_REPORT_PKG.APPLY_ERRORS(
                            p_run_id     => p_run_id,
                            p_errors     => l_ir_errors,
                            p_table_name => 'DMT_PJC_EXPENDITURES_TFM_TBL',
                            p_key_column => 'ORIG_TRANSACTION_REFERENCE');
