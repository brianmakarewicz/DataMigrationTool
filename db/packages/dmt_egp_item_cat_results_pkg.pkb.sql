...
-- ============================================================
    ...
    BEGIN
        ...
        --   literal '[RECONCILE_ERROR] ' tag.>>
                   '[RECONCILE_ERROR] Item category assignment not confirmed in Fusion '
                   || '(not found in the EGP_ITEM_CATEGORIES base table) after '
        ...
        -- (EDIT-SCOPE deleted — DMT_EGP_ITEM_CAT_TFM_TBL is not shared.)
        ;
    ...
    BEGIN
        ...
        BEGIN
            l_load_ess_id := DMT_LOADER_PKG.SUBMIT_LOAD(
                ...
                p_log_context       => 'ItemCategories'
            );
        EXCEPTION
            WHEN OTHERS THEN
                l_errmsg := SQLERRM;
                DMT_UTIL_PKG.LOG_ERROR(p_run_id,
                    'SUBMIT_LOAD failed for ItemCategories.', l_errmsg, C_PKG, C_PROC);
        ...
        DMT_LOADER_PKG.POLL_ESS_JOB(
            ...
            p_log_context    => 'ItemCategories > POLL_LOAD',
            p_cemli_code     => 'ItemCategories',
        ...
        IF l_fusion_status IN ('SUCCEEDED', 'WARNING') THEN
            BEGIN
                l_import_ess_id := DMT_LOADER_PKG.GET_IMPORT_ESS_ID(
                    p_run_id, 'ItemCategories', l_load_ess_id
                );
                ...
                DMT_LOADER_PKG.POLL_ESS_JOB(
                    ...
                    p_timeout_sec    => 1200,
                    p_raise_on_error => FALSE,
                    p_log_context    => 'ItemCategories > POLL_IMPORT',
                    p_cemli_code     => 'ItemCategories',
