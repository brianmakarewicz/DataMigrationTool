-- PACKAGE BODY DMT_EGP_ITEM_CAT_RUNNER_PKG

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DMT_EGP_ITEM_CAT_RUNNER_PKG" AS
-- ============================================================
-- DMT_EGP_ITEM_CAT_RUNNER_PKG Body
-- ============================================================

    C_PKG CONSTANT VARCHAR2(50) := 'DMT_EGP_ITEM_CAT_RUNNER_PKG';
    ...
    BEGIN
        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'Item Categories pipeline start. run_mode=' || p_run_mode
        ...
        -- Step 1: Pre-validate
        DMT_EGP_ITEM_CAT_VALIDATOR_PKG.VALIDATE_PRE_TRANSFORM(
            p_run_id   => p_run_id,
        ...
        DMT_EGP_ITEM_CAT_VALIDATOR_PKG.VALIDATE_POST_TRANSFORM(
            p_run_id => p_run_id
        );

        -- Step 4: Generate FBDI zip
        DMT_EGP_ITEM_CAT_FBDI_GEN_PKG.GENERATE_FBDI(
            p_run_id => p_run_id,
        ...
        IF l_fbdi_zip IS NOT NULL THEN
            ...
            DMT_EGP_ITEM_CAT_RESULTS_PKG.LOAD_AND_RECONCILE(
                p_run_id => p_run_id,
        ...
        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'Item Categories pipeline complete. TFM rows: ' || l_row_count,
            p_package        => C_PKG,
            p_procedure      => C_PROC);
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DMT_UTIL_PKG.LOG_ERROR(
                p_run_id => p_run_id,
                p_message        => 'Item Categories pipeline failed.',
                p_sqlerrm        => SQLERRM,
                p_package        => C_PKG,
                p_procedure      => C_PROC);
            RAISE;
