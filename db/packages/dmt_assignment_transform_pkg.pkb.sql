...
-- ============================================================
    ...
    BEGIN
        ...
        INSERT INTO DMT_WORK_REL_TFM_TBL (
            TFM_SEQUENCE_ID,
            STG_SEQUENCE_ID,
            RUN_ID,
            FBDI_CSV_ID,
    ...
    BEGIN
        ...
        INSERT INTO DMT_ASSIGNMENT_TFM_TBL (
            TFM_SEQUENCE_ID,
            STG_SEQUENCE_ID,
            RUN_ID,
            FBDI_CSV_ID,
        ...
        SELECT
            DMT_ASSIGNMENT_TFM_SEQ.NEXTVAL,
            s.STG_SEQUENCE_ID,
            p_run_id,
            NULL,
            s.EFFECTIVE_START_DATE,
        ...
        -- item). ALL/FAILED modes do not filter on STG_STATUS, so without this a row the
        -- validator rejected (missing ASSIGNMENT_NUMBER) would still be transformed.
        -- Scope to this object's SUB_OBJECT since STG_SEQUENCE_ID restarts per STG table.
        AND NOT EXISTS (
            SELECT 1 FROM DMT_STG_TFM_ERROR_TBL e
            WHERE  e.RUN_ID          = p_run_id
            AND    e.STG_SEQUENCE_ID = s.STG_SEQUENCE_ID
            AND    e.SUB_OBJECT      = 'Assignments'
