...
-- ============================================================
    ...
    -- --------------------------------------------------------
    -- APPLY_CONTRACT_V1_WORKSCHEDULES (private)
    -- The Contract v1 base-tier positive proof for the WorkSchedule record (design
    -- section 5), Option A shape (owner decision on PR #248). The shared package
    -- DMT_RECON_CONTRACT_PKG.FETCH_ROWS runs the WorkSchedules recon report over BIP
    -- and returns the parsed rows (no dynamic SQL, no TFM reference there); the APPLY
    -- here is STATIC SQL against the compile-time-known WorkSchedule TFM table. It
    -- confirms each migrated work schedule in the Fusion base view
    -- HTS_WORK_PATTERNS_VL by the prefixed work pattern name and marks that
    -- WorkSchedule TFM row LOADED with the real Fusion WORK_PATTERN_ID stamped into
    -- FUSION_SCHEDULE_ID; any ERROR row is marked FAILED with the real Fusion error.
    -- This REPLACES the bulk LOOKUP_FUSION_IDS positive path for WorkSchedules. The
    -- HDL data set request id is the Contract v1 P_LOAD_REQUEST_ID. Mirrors the
    -- Workers template (DMT_WORKER_RESULTS_PKG).
    -- --------------------------------------------------------
    PROCEDURE APPLY_CONTRACT_V1_WORKSCHEDULES (
        p_run_id     IN NUMBER,
        p_request_id IN VARCHAR2
    ) IS
        C_PROC      CONSTANT VARCHAR2(30) := 'APPLY_CONTRACT_V1_WS';
    ...
    BEGIN
        ...
        IF l_err_code <> DMT_UTIL_PKG.C_SUCCESS THEN
            RAISE_APPLICATION_ERROR(-20094,
                'APPLY_CONTRACT_V1_WORKSCHEDULES: Contract v1 fetch failed for '
                || 'WorkSchedules (detail in DMT_LOG_TBL).');
        END IF;
        ...
        IF l_rows.COUNT = 0 THEN
            ...
            DMT_UTIL_PKG.LOG(
                p_run_id    => p_run_id,
                p_message   => C_PROC || ': WorkSchedules recon report returned zero '
            ...
            FOR i IN 1 .. l_rows.COUNT LOOP
                IF l_rows(i).SOURCE_TYPE = 'BASE'
                   ...
                   AND l_rows(i).FUSION_ID IS NOT NULL THEN
                    -- Positive proof: work pattern found in HTS_WORK_PATTERNS_VL
                    -- with a real id. The ONLY path to LOADED. Static UPDATE.
                    UPDATE DMT_WORK_SCHED_TFM_TBL
                    SET    TFM_STATUS           = 'LOADED',
                           FUSION_SCHEDULE_ID   = l_rows(i).FUSION_ID,
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DMT_UTIL_PKG.LOG_ERROR(
                ...
                p_procedure => C_PROC);
            RAISE;
    END APPLY_CONTRACT_V1_WORKSCHEDULES;
    ...
    BEGIN
        ...
        -- 1. WorkSchedule — Contract v1 base-table proof (design section 5).
        ...
        -- Contract v1 parser below: a WorkSchedule row reaches LOADED only when the
        -- work pattern is positively confirmed in the Fusion base view
        -- (HTS_WORK_PATTERNS_VL) with a real WORK_PATTERN_ID, which the parser
        -- stamps into FUSION_SCHEDULE_ID.
        ...
        -- as STATIC SQL against the compile-time-known WorkSchedule TFM table. This
        -- REPLACES the prior LOOKUP_FUSION_IDS positive path for WorkSchedules.
        APPLY_CONTRACT_V1_WORKSCHEDULES(p_run_id, p_request_id);
