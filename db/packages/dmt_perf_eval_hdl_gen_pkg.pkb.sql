...
-- ============================================================
    ...
    BEGIN
        ...
        CASE p_tbl
            WHEN 'DMT_PERF_EVAL_TFM_TBL' THEN
                SELECT COUNT(*) INTO l_cnt FROM DMT_PERF_EVAL_TFM_TBL
                 WHERE RUN_ID = p_iid AND TFM_STATUS = 'STAGED' AND ROWNUM = 1;
            WHEN 'DMT_PERF_EVAL_RATING_TFM_TBL' THEN
                SELECT COUNT(*) INTO l_cnt FROM DMT_PERF_EVAL_RATING_TFM_TBL
                 WHERE RUN_ID = p_iid AND TFM_STATUS = 'STAGED' AND ROWNUM = 1;
            ELSE
