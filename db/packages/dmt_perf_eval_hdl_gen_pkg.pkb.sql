...
-- ============================================================
    ...
    -- The rating participant is the manager (manager provides the evaluation ratings).
    C_ROLE_MANAGER CONSTANT VARCHAR2(30) := 'Manager';
    ...
    BEGIN
        ...
        IF has_rows('DMT_PERF_EVAL_RATING_TFM_TBL', p_run_id) THEN
            ...
            FOR r IN (
                ...
                -- ParticipantPersonNumber = the manager, who supplies the ratings.
                -- ParticipantRoleTypeCode = Manager.
                ...
                l_vals := pv(r.PERSON_NUMBER)          || '|' ||  -- AssignmentNumber
                          pv(r.DOCUMENT_NAME)          || '|' ||  -- CustomaryName
                          pv(r.PERSON_NUMBER)          || '|' ||  -- ParticipantPersonNumber (manager rates; see note)
                          C_ROLE_MANAGER               || '|' ||  -- ParticipantRoleTypeCode
