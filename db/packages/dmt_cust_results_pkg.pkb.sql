...
-- ============================================================
    ...
    BEGIN
        ...
        BEGIN
            ...
            END IF;
        END;
        -- Results are written to the TFM tier only (FUSION_* ids, TFM_STATUS,
        -- appended ERROR_TEXT). The former echo-back that mirrored every
        -- outcome onto all 7 STG tables (setting STG_STATUS='LOADED'/'FAILED')
        -- is removed: staging is raw source data and never carries run-specific
        -- outcome state (design section 7; mirrors the Stage D Suppliers results
        -- conformance fix, whose reconciler writes no STG table).
