...
-- ============================================================
    ...
    --   STG_SEQUENCE_ID  identity PK, generated by the DB
    --   STAGE_DATE       DEFAULT SYSDATE
    --   STG_STATUS       DEFAULT 'NEW' NOT NULL (staging lifecycle)
    --   ERROR_TEXT       accumulated by the pipeline
    --   LAST_UPDATED_DATE audit column
    --   SCENARIO_ID      set by the upload package AFTER the load (tags the
    --                    newly inserted rows with the run's scenario); every
    --                    DMT_*_STG_TBL carries it, so it must be treated as an
    --                    admin column or the loader would offer it as an
    --                    uploadable business column (added 2026-07-16).
    -- SOURCE_ID is listed here so the dictionary marks it admin for the
    -- default (no-column-mapping) case, but it is a real business/natural key:
    -- when a CSV supplies a SOURCE_ID header the loader honours it (see
    -- DMT_CSV_UPLOAD_PKG, which unions SOURCE_ID back into the load column set).
    C_ADMIN_COLUMNS CONSTANT VARCHAR2(4000) :=
        ',STG_SEQUENCE_ID,SOURCE_ID,STAGE_DATE,STG_STATUS,ERROR_TEXT,LAST_UPDATED_DATE,SCENARIO_ID,';
