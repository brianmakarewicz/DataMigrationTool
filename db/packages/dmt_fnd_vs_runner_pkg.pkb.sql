-- PACKAGE BODY DMT_FND_VS_RUNNER_PKG

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DMT_FND_VS_RUNNER_PKG" AS
-- ============================================================
-- DMT_FND_VS_RUNNER_PKG Body
-- Orchestrates the full Value Set pipeline for one integration run.
-- ============================================================

    C_PKG CONSTANT VARCHAR2(50) := 'DMT_FND_VS_RUNNER_PKG';
    ...
    BEGIN
        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'FND Value Set pipeline start. run_mode=' || p_run_mode
        ...
        DMT_FND_VS_VALIDATOR_PKG.VALIDATE_PRE_TRANSFORM(
            p_run_id   => p_run_id,
            p_dependent_prefix => NULL
        );

        -- Step 2: Transform sets (STG -> TFM)
        DMT_FND_VS_TRANSFORM_PKG.TRANSFORM_SETS(
            p_run_id   => p_run_id,
        ...
        DMT_FND_VS_TRANSFORM_PKG.TRANSFORM_VALUES(
            p_run_id   => p_run_id,
            p_reprocess_errors => l_reprocess,
            p_scenario_id      => p_scenario_id,
            p_include_untagged => p_include_untagged,
        ...
        DMT_FND_VS_VALIDATOR_PKG.VALIDATE_POST_TRANSFORM(
            p_run_id => p_run_id
        );

        -- Step 5: Generate FBL zip
        DMT_FND_VS_FBL_GEN_PKG.GENERATE_FBL(
            p_run_id => p_run_id,
            x_fbl_zip        => l_fbl_zip,
        ...
        IF l_fbl_zip IS NOT NULL THEN
            DMT_FND_VS_RESULTS_PKG.LOAD_AND_RECONCILE(
                p_run_id => p_run_id
            );
        END IF;
        ...
        DMT_UTIL_PKG.LOG(
            p_run_id => p_run_id,
            p_message        => 'FND Value Set pipeline complete. Sets: ' || l_set_count
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DMT_UTIL_PKG.LOG_ERROR(
                p_run_id => p_run_id,
                p_message        => 'FND Value Set pipeline failed.',
                p_sqlerrm        => SQLERRM,
                p_package        => C_PKG,
                p_procedure      => C_PROC);
            RAISE;
