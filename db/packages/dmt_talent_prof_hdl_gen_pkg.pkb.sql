...
-- ============================================================
    ...
    BEGIN
        ...
        CASE p_tbl
            WHEN 'DMT_TALENT_PROF_TFM_TBL' THEN
                SELECT COUNT(*) INTO l_cnt FROM DMT_TALENT_PROF_TFM_TBL
                 WHERE RUN_ID = p_iid AND TFM_STATUS = 'STAGED' AND ROWNUM = 1;
            WHEN 'DMT_TALENT_PROF_ITEM_TFM_TBL' THEN
                SELECT COUNT(*) INTO l_cnt FROM DMT_TALENT_PROF_ITEM_TFM_TBL
                 WHERE RUN_ID = p_iid AND TFM_STATUS = 'STAGED' AND ROWNUM = 1;
            ELSE
    ...
    BEGIN
        ...
        IF DBMS_LOB.GETLENGTH(l_dat) > 0 THEN
            UTL_ZIP.add1file(l_zip, 'TalentProfile.dat',
                clob_to_blob(l_dat));
        END IF;
