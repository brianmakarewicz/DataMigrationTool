...
-- ============================================================
    ...
    BEGIN
        ...
        AND NOT EXISTS (
            SELECT 1 FROM DMT_PO_HEADERS_INT_TFM_TBL t
    ...
    BEGIN
        ...
        SELECT
                    ...
                    s.LINE_NUM,
                    s.LINE_TYPE,
                    DMT_XREF_PKG.ITEM_NUMBER(s.ITEM),
                    s.ITEM_DESCRIPTION,
                    s.ITEM_REVISION,
                    s.CATEGORY,
                    s.AMOUNT,
        ...
        AND (p_doc_type_filter IS NULL
             OR EXISTS (
                SELECT 1 FROM DMT_PO_HEADERS_INT_STG_TBL h
    ...
    BEGIN
        ...
        AND (p_scenario_id IS NULL
             ...
             OR (p_include_untagged = 'Y' AND s.SCENARIO_ID IS NULL))
        AND NOT EXISTS (
            SELECT 1 FROM DMT_PO_LINE_LOCS_INT_TFM_TBL t
            WHERE  t.STG_SEQUENCE_ID = s.STG_SEQUENCE_ID
            AND    t.RUN_ID  = p_run_id
        )
        -- Honor pre-validation rejections in EVERY run mode (ALL-mode-bypass backlog
    ...
    BEGIN
        ...
        SELECT
                    ...
                    DMT_XREF_PKG.PROJECT_NUMBER(s.PROJECT),
                    DMT_XREF_PKG.TASK_NUMBER(s.TASK),
