-- PACKAGE BODY DMT_FND_LOOKUP_VALIDATOR_PKG

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DMT_FND_LOOKUP_VALIDATOR_PKG" 
AS
-- ============================================================
-- DMT_FND_LOOKUP_VALIDATOR_PKG body
-- Lookup Types and Values pre- and post-transform validation.
-- ============================================================

    C_PKG CONSTANT VARCHAR2(50) := 'DMT_FND_LOOKUP_VALIDATOR_PKG';

    -- --------------------------------------------------------
    -- VALIDATE_PRE_TRANSFORM
    -- Upstream dependency check before transformation.
    -- Lookups have no upstream dependencies — stub for now.
    -- --------------------------------------------------------
    -- ============================================================
    ...
    BEGIN
        ...
        -- Lookups are standalone master data with no upstream dependencies.
        NULL;
    ...
    -- TFM type row (matched on LOOKUP_TYPE within the same
    ...
    BEGIN
        ...
        -- Mark orphan value rows whose LOOKUP_TYPE has no matching type row
        UPDATE DMT_OWNER.DMT_FND_LOOKUP_VALUE_TFM_TBL v
        SET    v.TFM_STATUS        = 'FAILED',
               v.ERROR_TEXT        = NVL2(v.ERROR_TEXT,
                                        v.ERROR_TEXT || ' | ',
                                        '')
                                    || '[POST_VALIDATION] Parent lookup type not found for LOOKUP_TYPE='
                                    || v.LOOKUP_TYPE,
        ...
        AND    NOT EXISTS (
            SELECT 1
            FROM   DMT_OWNER.DMT_FND_LOOKUP_TYPE_TFM_TBL t
