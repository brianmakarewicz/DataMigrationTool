...
-- numbering schemes (DMT_POZ_SUPPLIERS_TFM_TBL_N* and DMT_POZ_SUPPLIERS_TFM_N* both in
...
declare
  ...
  l_cols varchar2(400);
begin
  select count(*) into l_n from user_indexes where index_name = 'DMT_POZ_SUPPLIERS_TFM_TBL_N1';
  if l_n > 0 then execute immediate 'DROP INDEX "DMT_POZ_SUPPLIERS_TFM_TBL_N1"'; end if;
  select count(*) into l_n from user_indexes where index_name = 'DMT_POZ_SUPPLIERS_TFM_TBL_N4';
  if l_n > 0 then execute immediate 'DROP INDEX "DMT_POZ_SUPPLIERS_TFM_TBL_N4"'; end if;
  select listagg(column_name,',') within group (order by column_position) into l_cols
    from user_ind_columns where index_name = 'DMT_POZ_SUPPLIERS_TFM_N3';
  if l_cols = 'RECON_KEY' then execute immediate 'DROP INDEX "DMT_POZ_SUPPLIERS_TFM_N3"'; end if;
end;
/

begin
  execute immediate 'CREATE INDEX "DMT_POZ_SUPPLIERS_TFM_N3" ON "DMT_POZ_SUPPLIERS_TFM_TBL" ("FBDI_CSV_ID")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
...
begin
  execute immediate 'CREATE INDEX "DMT_POZ_SUPPLIERS_TFM_N4" ON "DMT_POZ_SUPPLIERS_TFM_TBL" ("STG_SEQUENCE_ID")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
...
begin
  execute immediate 'CREATE INDEX "DMT_POZ_SUPPLIERS_TFM_N5" ON "DMT_POZ_SUPPLIERS_TFM_TBL" ("RECON_KEY")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
...
/
COMMENT ON COLUMN "DMT_POZ_SUPPLIERS_TFM_TBL"."WORK_QUEUE_ID" IS 'The work queue item (DMT_WORK_QUEUE_TBL.QUEUE_ID) that processed this record. FK in _foreign_keys.sql. Stamped at generation; unit of per-work-item processing (design section 7, accepted 2026-07-20).';
