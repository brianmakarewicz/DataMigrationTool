-- DMT_INV_TRX_STG_TBL (generated from ATP 2026-07-03)

begin
  execute immediate 'CREATE TABLE "DMT_INV_TRX_STG_TBL" 
   (	"STG_SEQUENCE_ID" NUMBER DEFAULT DMT_OWNER.DMT_INV_TRX_STG_SEQ.NEXTVAL NOT NULL ENABLE, 
	...
	"SCENARIO_ID" NUMBER, 
	 CONSTRAINT "DMT_INV_TRX_STG_PK" PRIMARY KEY ("STG_SEQUENCE_ID")
  USING INDEX  ENABLE
   ) ';
...
begin
  ...
  where  table_name = 'DMT_INV_TRX_STG_TBL' and column_name = 'STATUS';
  if l_n = 1 then
    execute immediate 'ALTER TABLE "DMT_INV_TRX_STG_TBL" RENAME COLUMN "STATUS" TO "STG_STATUS"';
  end if;
end;
/
...
begin
  select count(*) into l_n from user_tab_columns
  where  table_name = 'DMT_INV_TRX_STG_TBL' and column_name = 'SCENARIO_ID';
  if l_n = 0 then
    execute immediate 'ALTER TABLE "DMT_INV_TRX_STG_TBL" ADD ("SCENARIO_ID" NUMBER)';
  end if;
end;
/
begin
  execute immediate 'CREATE INDEX "DMT_INV_TRX_STG_N1" ON "DMT_INV_TRX_STG_TBL" ("STG_STATUS")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
begin
  execute immediate 'CREATE INDEX "DMT_INV_TRX_STG_N2" ON "DMT_INV_TRX_STG_TBL" ("SCENARIO_ID")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/

COMMENT ON COLUMN "DMT_INV_TRX_STG_TBL"."STG_STATUS" IS 'Staging lifecycle: NEW > TRANSFORMED / FAILED. Forward-only, never reset; errors accumulate in ERROR_TEXT.';
...
begin
  select nullable into l_nullable from user_tab_columns
   where table_name = 'DMT_INV_TRX_STG_TBL' and column_name = 'STG_STATUS';
  if l_nullable = 'Y' then
    execute immediate 'UPDATE "DMT_INV_TRX_STG_TBL" SET "STG_STATUS" = ''NEW'' WHERE "STG_STATUS" IS NULL';
    execute immediate 'ALTER TABLE "DMT_INV_TRX_STG_TBL" MODIFY ("STG_STATUS" DEFAULT ''NEW'' NOT NULL)';
  end if;
end;
/
