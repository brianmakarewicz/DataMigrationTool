...
begin
  select nullable into l_nullable from user_tab_columns
   where table_name = 'DMT_POZ_SUPPLIERS_STG_TBL' and column_name = 'STG_STATUS';
  if l_nullable = 'Y' then
    execute immediate 'UPDATE "DMT_POZ_SUPPLIERS_STG_TBL" SET "STG_STATUS" = ''NEW'' WHERE "STG_STATUS" IS NULL';
    execute immediate 'ALTER TABLE "DMT_POZ_SUPPLIERS_STG_TBL" MODIFY ("STG_STATUS" DEFAULT ''NEW'' NOT NULL)';
  end if;
end;
/
...
-- numbering schemes (DMT_POZ_SUPPLIERS_STG_TBL_N* and DMT_POZ_SUPPLIERS_STG_N* both in
...
declare
  ...
  l_cols varchar2(400);
begin
  select count(*) into l_n from user_indexes where index_name = 'DMT_POZ_SUPPLIERS_STG_TBL_N1';
  if l_n > 0 then execute immediate 'DROP INDEX "DMT_POZ_SUPPLIERS_STG_TBL_N1"'; end if;
  select listagg(column_name,',') within group (order by column_position) into l_cols
    from user_ind_columns where index_name = 'DMT_POZ_SUPPLIERS_STG_N1';
  if l_cols = 'SCENARIO_ID' then execute immediate 'DROP INDEX "DMT_POZ_SUPPLIERS_STG_N1"'; end if;
end;
/

begin
  execute immediate 'CREATE INDEX "DMT_POZ_SUPPLIERS_STG_N1" ON "DMT_POZ_SUPPLIERS_STG_TBL" ("STG_STATUS")';
exception when others then
...
begin
  execute immediate 'CREATE INDEX "DMT_POZ_SUPPLIERS_STG_N2" ON "DMT_POZ_SUPPLIERS_STG_TBL" ("SCENARIO_ID")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
