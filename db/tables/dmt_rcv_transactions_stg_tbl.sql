-- DMT_RCV_TRANSACTIONS_STG_TBL (generated from ATP 2026-07-03)

begin
  execute immediate 'CREATE TABLE "DMT_RCV_TRANSACTIONS_STG_TBL" 
   (	"STG_SEQUENCE_ID" NUMBER DEFAULT DMT_OWNER.DMT_RCV_TRANSACTIONS_STG_SEQ.NEXTVAL NOT NULL ENABLE, 
	...
	"SCENARIO_ID" NUMBER, 
	 CONSTRAINT "DMT_RCV_TRANSACTIONS_STG_PK" PRIMARY KEY ("STG_SEQUENCE_ID")
  USING INDEX  ENABLE
   ) ';
...
begin
  ...
  where  table_name = 'DMT_RCV_TRANSACTIONS_STG_TBL' and column_name = 'STATUS';
  if l_n = 1 then
    execute immediate 'ALTER TABLE "DMT_RCV_TRANSACTIONS_STG_TBL" RENAME COLUMN "STATUS" TO "STG_STATUS"';
  end if;
end;
/

begin
  execute immediate 'CREATE INDEX "DMT_RCV_TXN_STG_N1" ON "DMT_RCV_TRANSACTIONS_STG_TBL" ("STG_STATUS")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/

COMMENT ON COLUMN "DMT_RCV_TRANSACTIONS_STG_TBL"."STG_SEQUENCE_ID" IS 'PK - from DMT_RCV_TRANSACTIONS_STG_SEQ';
COMMENT ON COLUMN "DMT_RCV_TRANSACTIONS_STG_TBL"."HEADER_INTERFACE_NUM" IS 'FK link to RcvHeadersInterface â€” matches HEADER_INTERFACE_NUM in DMT_RCV_HEADERS_STG_TBL';
COMMENT ON COLUMN "DMT_RCV_TRANSACTIONS_STG_TBL"."TO_ORGANIZATION_CODE" IS 'Destination inventory organization code';
COMMENT ON COLUMN "DMT_RCV_TRANSACTIONS_STG_TBL"."ITEM_NUM" IS 'Item number â€” validated against inventory org items';
COMMENT ON COLUMN "DMT_RCV_TRANSACTIONS_STG_TBL"."ERROR_TEXT" IS 'Concatenated errors â€” appended at each step, never overwritten. Prefixed: [PRE_VALIDATION] [TRANSFORM_ERROR] [FUSION_ERROR].';
COMMENT ON TABLE "DMT_RCV_TRANSACTIONS_STG_TBL"  IS 'Receiving transaction staging (MiscReceipts / Items on Hand). Raw user data only. Run-specific data in DMT_RCV_TRANSACTIONS_TFM_TBL. FBDI interface: RCV_TRANSACTIONS_INTERFACE. CSV: RcvTransactionsInterface.csv.';
...
-- ---------------------------------------------------------------------------
begin
  execute immediate 'CREATE INDEX "DMT_RCV_TRANSACTIONS_STG_N1" ON "DMT_RCV_TRANSACTIONS_STG_TBL" ("SCENARIO_ID")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/

COMMENT ON COLUMN "DMT_RCV_TRANSACTIONS_STG_TBL"."STG_STATUS" IS 'Staging lifecycle: NEW > TRANSFORMED / FAILED. Forward-only, never reset; errors accumulate in ERROR_TEXT.';
...
begin
  select nullable into l_nullable from user_tab_columns
   where table_name = 'DMT_RCV_TRANSACTIONS_STG_TBL' and column_name = 'STG_STATUS';
  if l_nullable = 'Y' then
    execute immediate 'UPDATE "DMT_RCV_TRANSACTIONS_STG_TBL" SET "STG_STATUS" = ''NEW'' WHERE "STG_STATUS" IS NULL';
    execute immediate 'ALTER TABLE "DMT_RCV_TRANSACTIONS_STG_TBL" MODIFY ("STG_STATUS" DEFAULT ''NEW'' NOT NULL)';
  end if;
end;
/
