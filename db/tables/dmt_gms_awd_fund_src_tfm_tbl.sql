...
begin
  ...
  where  table_name = 'DMT_GMS_AWD_FUND_SRC_TFM_TBL' and column_name = 'STATUS';
  if l_n = 1 then
    execute immediate 'ALTER TABLE "DMT_GMS_AWD_FUND_SRC_TFM_TBL" RENAME COLUMN "STATUS" TO "TFM_STATUS"';
  end if;
end;
/
...
begin
  select count(*) into l_n from user_tab_columns
  where  table_name = 'DMT_GMS_AWD_FUND_SRC_TFM_TBL' and column_name = 'RECON_KEY';
  if l_n = 0 then
    execute immediate 'ALTER TABLE "DMT_GMS_AWD_FUND_SRC_TFM_TBL" ADD ("RECON_KEY" VARCHAR2(1000))';
  end if;
end;
/
declare
...
begin
  select count(*) into l_n from user_tab_columns
  where  table_name = 'DMT_GMS_AWD_FUND_SRC_TFM_TBL' and column_name = 'FUSION_AWARD_ID';
  if l_n = 0 then
    execute immediate 'ALTER TABLE "DMT_GMS_AWD_FUND_SRC_TFM_TBL" ADD ("FUSION_AWARD_ID" NUMBER)';
  end if;
end;
/
begin
  execute immediate 'CREATE INDEX "DMT_GMS_AWD_FUND_SRC_TFM_N1" ON "DMT_GMS_AWD_FUND_SRC_TFM_TBL" ("RUN_ID")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
begin
  execute immediate 'CREATE INDEX "DMT_GMS_AWD_FUND_SRC_TFM_N2" ON "DMT_GMS_AWD_FUND_SRC_TFM_TBL" ("RUN_ID", "TFM_STATUS")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
begin
  execute immediate 'CREATE INDEX "DMT_GMS_AWD_FUND_SRC_TFM_N3" ON "DMT_GMS_AWD_FUND_SRC_TFM_TBL" ("FBDI_CSV_ID")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
begin
  execute immediate 'CREATE INDEX "DMT_GMS_AWD_FUND_SRC_TFM_N4" ON "DMT_GMS_AWD_FUND_SRC_TFM_TBL" ("RECON_KEY")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
