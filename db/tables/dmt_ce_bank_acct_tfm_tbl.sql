...
begin
  execute immediate 'CREATE TABLE "DMT_CE_BANK_ACCT_TFM_TBL" 
   (	"TFM_SEQUENCE_ID" NUMBER DEFAULT DMT_CE_BANK_ACCT_TFM_SEQ.NEXTVAL NOT NULL ENABLE, 
...
begin
  select nullable into l_nullable from user_tab_columns
   where table_name = 'DMT_CE_BANK_ACCT_TFM_TBL' and column_name = 'TFM_STATUS';
  if l_nullable = 'Y' then
    execute immediate 'UPDATE "DMT_CE_BANK_ACCT_TFM_TBL" SET "TFM_STATUS" = ''STAGED'' WHERE "TFM_STATUS" IS NULL';
    execute immediate 'ALTER TABLE "DMT_CE_BANK_ACCT_TFM_TBL" MODIFY ("TFM_STATUS" DEFAULT ''STAGED'' NOT NULL)';
  end if;
end;
/
...
/
COMMENT ON COLUMN "DMT_CE_BANK_ACCT_TFM_TBL"."WORK_QUEUE_ID" IS 'The work queue item (DMT_WORK_QUEUE_TBL.QUEUE_ID) that processed this record. FK in _foreign_keys.sql. Stamped at generation; unit of per-work-item processing (design section 7, accepted 2026-07-20).';
