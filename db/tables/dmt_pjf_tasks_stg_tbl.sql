...
begin
  ...
  where  table_name = 'DMT_PJF_TASKS_STG_TBL';
  if l_exists = 1 then
    select count(*) into l_identity from user_tab_identity_cols
    where  table_name = 'DMT_PJF_TASKS_STG_TBL' and column_name = 'STG_SEQUENCE_ID';
    if l_identity = 0 then
      execute immediate 'select count(*) from "DMT_PJF_TASKS_STG_TBL" where rownum = 1' into l_rows;
      if l_rows = 0 then
        execute immediate 'drop table "DMT_PJF_TASKS_STG_TBL" cascade constraints purge';
      else
        raise_application_error(-20001,
          'DMT_PJF_TASKS_STG_TBL: cannot convert a populated table to an identity PK in place (ORA-30673). ' ||
...
begin
  ...
  where  table_name = 'DMT_PJF_TASKS_STG_TBL' and column_name = 'STATUS';
  if l_n = 1 then
    execute immediate 'ALTER TABLE "DMT_PJF_TASKS_STG_TBL" RENAME COLUMN "STATUS" TO "STG_STATUS"';
  end if;
end;
/

begin
  execute immediate 'CREATE INDEX "DMT_PJF_TASKS_STG_TBL_N1" ON "DMT_PJF_TASKS_STG_TBL" ("STG_STATUS")';
exception when others then
...
-- ---------------------------------------------------------------------------
begin
  execute immediate 'CREATE INDEX "DMT_PJF_TASKS_STG_N1" ON "DMT_PJF_TASKS_STG_TBL" ("SCENARIO_ID")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/

COMMENT ON COLUMN "DMT_PJF_TASKS_STG_TBL"."STG_STATUS" IS 'Staging lifecycle: NEW > TRANSFORMED / FAILED. Forward-only, never reset; errors accumulate in ERROR_TEXT.';
...
begin
  select nullable into l_nullable from user_tab_columns
   where table_name = 'DMT_PJF_TASKS_STG_TBL' and column_name = 'STG_STATUS';
  if l_nullable = 'Y' then
    execute immediate 'UPDATE "DMT_PJF_TASKS_STG_TBL" SET "STG_STATUS" = ''NEW'' WHERE "STG_STATUS" IS NULL';
    execute immediate 'ALTER TABLE "DMT_PJF_TASKS_STG_TBL" MODIFY ("STG_STATUS" DEFAULT ''NEW'' NOT NULL)';
  end if;
end;
/
