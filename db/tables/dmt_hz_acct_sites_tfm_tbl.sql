...
begin
  ...
  where  table_name = 'DMT_HZ_ACCT_SITES_TFM_TBL';
  if l_exists = 1 then
    select count(*) into l_identity from user_tab_identity_cols
    where  table_name = 'DMT_HZ_ACCT_SITES_TFM_TBL' and column_name = 'TFM_SEQUENCE_ID';
    if l_identity = 0 then
      execute immediate 'select count(*) from "DMT_HZ_ACCT_SITES_TFM_TBL" where rownum = 1' into l_rows;
      if l_rows = 0 then
        execute immediate 'drop table "DMT_HZ_ACCT_SITES_TFM_TBL" cascade constraints purge';
      else
        raise_application_error(-20001,
          'DMT_HZ_ACCT_SITES_TFM_TBL: cannot convert a populated table to an identity PK in place (ORA-30673). ' ||
...
/
COMMENT ON COLUMN "DMT_HZ_ACCT_SITES_TFM_TBL"."WORK_QUEUE_ID" IS 'The work queue item (DMT_WORK_QUEUE_TBL.QUEUE_ID) that processed this record. FK in _foreign_keys.sql. Stamped at generation; unit of per-work-item processing (design section 7, accepted 2026-07-20).';
