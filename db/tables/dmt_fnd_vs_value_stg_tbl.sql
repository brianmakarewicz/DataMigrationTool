-- DMT_FND_VS_VALUE_STG_TBL (generated from ATP 2026-07-03)

begin
  execute immediate 'CREATE TABLE "DMT_FND_VS_VALUE_STG_TBL" 
   (	"STG_SEQUENCE_ID" NUMBER DEFAULT DMT_OWNER.DMT_FND_VS_VALUE_STG_SEQ.NEXTVAL NOT NULL ENABLE, 
	...
	"SCENARIO_ID" NUMBER, 
	 CONSTRAINT "DMT_FND_VS_VALUE_STG_PK" PRIMARY KEY ("STG_SEQUENCE_ID")
  USING INDEX  ENABLE
   ) ';
...
begin
  ...
  where  table_name = 'DMT_FND_VS_VALUE_STG_TBL' and column_name = 'STATUS';
  if l_n = 1 then
    execute immediate 'ALTER TABLE "DMT_FND_VS_VALUE_STG_TBL" RENAME COLUMN "STATUS" TO "STG_STATUS"';
  end if;
end;
/

COMMENT ON TABLE "DMT_FND_VS_VALUE_STG_TBL"  IS 'Flex Value Set Values staging. Raw user-loaded data, child of FND_VS_SET. Run-specific data in DMT_FND_VS_VALUE_TFM_TBL.';
...
-- ---------------------------------------------------------------------------
begin
  execute immediate 'CREATE INDEX "DMT_FND_VS_VALUE_STG_N1" ON "DMT_FND_VS_VALUE_STG_TBL" ("STG_STATUS")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
begin
  execute immediate 'CREATE INDEX "DMT_FND_VS_VALUE_STG_N2" ON "DMT_FND_VS_VALUE_STG_TBL" ("SCENARIO_ID")';
exception when others then
  if sqlcode not in (-955,-1408) then raise; end if;
end;
/
