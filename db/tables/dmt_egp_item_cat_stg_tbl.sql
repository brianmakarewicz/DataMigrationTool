...
begin
  execute immediate 'CREATE TABLE "DMT_EGP_ITEM_CAT_STG_TBL" 
   (	"STG_SEQUENCE_ID" NUMBER DEFAULT DMT_EGP_ITEM_CAT_STG_SEQ.NEXTVAL NOT NULL ENABLE, 
...
begin
  select nullable into l_nullable from user_tab_columns
   where table_name = 'DMT_EGP_ITEM_CAT_STG_TBL' and column_name = 'STG_STATUS';
  if l_nullable = 'Y' then
    execute immediate 'UPDATE "DMT_EGP_ITEM_CAT_STG_TBL" SET "STG_STATUS" = ''NEW'' WHERE "STG_STATUS" IS NULL';
    execute immediate 'ALTER TABLE "DMT_EGP_ITEM_CAT_STG_TBL" MODIFY ("STG_STATUS" DEFAULT ''NEW'' NOT NULL)';
  end if;
end;
/
