...
-- ---------------------------------------------------------------------------
-- Absences — Contract v1 registration (design section 5). Copied EXACTLY from
-- the Workers block above (PR #248). Registers the four Contract v1 columns
-- (CONTRACT_VERSION, TFM_TABLE, FUSION_ID_COLUMN, RECON_KEY_SQL) that drive the
-- shared parser DMT_RECON_CONTRACT_PKG.FETCH_ROWS. Kept in its own MERGE (on
-- BIP_REPORT_ID) so re-running just this block converges the row without the
-- ORA-00001 risk of re-running the whole seed. HDL load = no interface table
-- (INTERFACE_TABLE = 'N/A (HDL)'); reconciliation is base-tier only from
-- ANC_PER_ABS_ENTRIES, tied to our written SourceSystemId through
-- HRC_INTEGRATION_KEY_MAP (OBJECT_NAME='PersonAbsenceEntry'). RECON_KEY = the
-- prefixed PERSON_NUMBER with the '_ABS' record suffix (also the .dat
-- SourceSystemId and the report RECORD_KEY).
-- ---------------------------------------------------------------------------
merge into "DMT_BIP_REPORT_TBL" t
using (
    select 100000030                                              bip_report_id,
           ...
           'DMT_UTIL_PKG.PREFIXED(run_prefix, PERSON_NUMBER, 30) || ''_ABS'' -- prefixed person number + absence-entry record suffix (matches .dat SourceSystemId)' recon_key_sql
    from dual
) s
on (t."BIP_REPORT_ID" = s.bip_report_id)
when matched then update set
