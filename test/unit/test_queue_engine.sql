...
begin
    ...
    -- ----------------------------------------------------------
    -- (h) 31-33. Accounting gate: the mock reconciler confirms no row
    --     (no positive success, no positive failure), so every row is
    --     left GENERATED by RECONCILE_BATCH. The shared unaccounted
    --     sweep (DMT_QUEUE_WORKER_PKG.SWEEP_UNACCOUNTED, called from
    --     RECONCILE_ONE after RECONCILE_BATCH) then flips those rows to
    --     the real terminal status UNACCOUNTED with the bare
    --     [UNACCOUNTED] tag. Either way the accounting gate counts them
    --     as unaccounted, so the item must be FAILED, never DONE.
    -- ----------------------------------------------------------
    ...
    -- for." Two rows swept to UNACCOUNTED = unaccounted -> FAILED.
    ...
    assert(instr(l_errmsg, 'unaccounted') > 0, 32,
        ...
        ||substr(l_errmsg,1,120)||')');
    -- The shared sweep set the real terminal status UNACCOUNTED on both rows
    -- (not left GENERATED), tagged with the bare [UNACCOUNTED] marker and nothing
    -- else. Prove both the status flip and the tag.
    select count(*) into l_cnt
      from dmt_mock_tfm_tbl
     where run_id = l_run_h and tfm_status = 'UNACCOUNTED';
    assert(l_cnt = 2, 311,
        'unaccounted sweep set both rows to TFM_STATUS UNACCOUNTED (got '||l_cnt||')');
    select count(*) into l_cnt
      from dmt_mock_tfm_tbl
     where run_id = l_run_h and tfm_status = 'UNACCOUNTED'
       and dbms_lob.instr(error_text, '[UNACCOUNTED]') > 0;
    assert(l_cnt = 2, 312,
        'unaccounted sweep appended the bare [UNACCOUNTED] tag to both rows (got '||l_cnt||')');
